...
@section('content')
    <div class="space-y-6 animate-fade-in">
        {{-- Welcome Banner --}}
        <div class="relative overflow-hidden rounded-2xl bg-theme-gradient p-6 lg:p-8 shadow-lg shadow-primary-500/20">
            ...
            <div class="absolute bottom-0 left-1/4 w-32 h-32 bg-white/10 rounded-full blur-2xl pointer-events-none"></div>
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <div class="inline-flex items-center gap-2 px-3 py-1 bg-white/20 backdrop-blur-md rounded-full mb-3 border border-white/10">
                        ...
                        <span class="text-xs text-white/90 font-medium tracking-wide">Sistem Berjalan Normal</span>
                    </div>
                    <h1 class="text-2xl lg:text-3xl font-bold text-white mb-2 leading-tight">Selamat Datang, {{ Auth::user()->name ?? 'Admin' }}! 👋</h1>
                    <p class="text-white/80 text-sm lg:text-base max-w-xl leading-relaxed">
                        Ringkasan aktivitas dan performa portal berita Anda hari ini.
                        @if($stats['draft_articles'] > 0)
                            <span class="inline-block bg-white/20 px-2 rounded ml-1 text-white font-medium">{{ $stats['draft_articles'] }} berita draft</span> menunggu review.
                        @endif
                    </p>
                </div>
                <div class="flex flex-wrap gap-3">
                    <a href="#" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-primary-600 rounded-xl font-bold hover:bg-surface-50 transition-all duration-200 shadow-lg shadow-black/10 hover:shadow-xl hover:-translate-y-0.5 text-sm">
                        <i data-lucide="pen-tool" class="w-4 h-4"></i>
                        <span>Tulis Berita</span>
                    </a>
                    @if($stats['draft_articles'] > 0)
                    <a href="#" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white/20 text-white rounded-xl font-semibold hover:bg-white/30 transition-all duration-200 backdrop-blur-md text-sm border border-white/10">
