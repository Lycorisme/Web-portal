<div class="rounded-[2rem] p-6 mb-14 relative overflow-hidden bg-slate-900/40 backdrop-blur-xl border border-white/5 shadow-2xl ring-1 ring-white/5 group">
    ...
    <div class="relative z-10 flex flex-col md:flex-row items-center gap-8">
        ...
        <div class="relative shrink-0 group/avatar">
            <div class="w-28 h-28 md:w-32 md:h-32 rounded-full p-1 bg-gradient-to-br from-emerald-500/30 to-purple-500/30 backdrop-blur-sm ring-1 ring-white/10 relative">
                <div class="w-full h-full rounded-full overflow-hidden bg-slate-800 relative">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user->profile_photo): ?>
                        <img src="<?php echo e($user->avatar); ?>" class="w-full h-full object-cover transition-transform duration-500 group-hover/avatar:scale-110" id="header-avatar">
                    <?php else: ?>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($user->profile_photo): ?>
                <button type="button" @click="submitForm('header-photo-delete-form', '<?php echo e(route('public.profile.photo.delete')); ?>')" 
            ...
            <form id="header-photo-form" enctype="multipart/form-data">
                <input type="file" name="profile_photo" id="header-photo-input" accept="image/*" class="hidden"
                       onchange="submitForm('header-photo-form', '<?php echo e(route('public.profile.photo')); ?>')">
            </form>
            <form id="header-photo-delete-form" class="hidden"><?php echo method_field('DELETE'); ?></form>
        </div>

        
        <div class="flex-1 min-w-0 text-center md:text-left">
            <h1 class="text-3xl md:text-4xl font-display font-bold text-white mb-1 tracking-tight" id="header-name"><?php echo e($user->name); ?></h1>
            <p class="text-slate-400 text-base mb-4 font-medium" id="header-email"><?php echo e($user->email); ?></p>
        ...
        <div class="flex items-center gap-8 md:border-l border-white/5 md:pl-8">
            <div class="text-center">
                <span class="block text-2xl font-bold text-white mb-0.5"><?php echo e($stats['likes_count']); ?></span>
                <span class="text-[10px] uppercase tracking-[0.15em] text-slate-500 font-bold">Likes</span>
            </div>
            <div class="text-center">
                 <span class="block text-2xl font-bold text-white mb-0.5"><?php echo e($stats['comments_count']); ?></span>
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\public\profile\partials\profile-header.blade.php ENDPATH**/ ?>