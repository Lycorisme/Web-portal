...
async bulkRestore() {
    ...
    try {
        const response = await fetch('<?php echo e(route("trash.bulk-restore")); ?>', {
            method: 'POST',
            headers: {
...
async bulkForceDelete() {
    ...
    try {
        const response = await fetch('<?php echo e(route("trash.bulk-force-delete")); ?>', {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Accept': 'application/json',
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\trash\partials\scripts\bulk-actions.blade.php ENDPATH**/ ?>