                ...
                <div class="bg-white dark:bg-surface-800 rounded-3xl overflow-hidden shadow-2xl border border-surface-100 dark:border-surface-700 transform transition-transform hover:scale-[1.02] duration-500">
                    ...
                    <div class="p-6 relative">
                        <div class="absolute -top-10 right-6 w-12 h-12 bg-white dark:bg-surface-800 rounded-full flex items-center justify-center shadow-lg border-2 border-white dark:border-surface-700 overflow-hidden">
                            <?php $logoUrl = \App\Models\SiteSetting::get('logo_url', ''); ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($logoUrl): ?>
                                <img src="<?php echo e($logoUrl); ?>" alt="Logo" class="w-full h-full object-cover">
                            <?php else: ?>
                                <span class="text-xs font-bold text-theme-600"><?php echo e(strtoupper(Str::limit(config('app.name', 'APP'), 5, ''))); ?></span>
                        ...
                        <p class="text-[10px] items-center flex gap-1 text-surface-400 uppercase tracking-widest font-bold mb-2">
                            <?php echo e(parse_url(config('app.url', 'example.com'), PHP_URL_HOST) ?? config('app.name')); ?> <span class="w-1 h-1 rounded-full bg-surface-300"></span> News
                        </p>
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\articles\partials\form-modal\tab-seo.blade.php ENDPATH**/ ?>