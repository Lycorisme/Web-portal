...
<template x-teleport="#main-content">
    ...
    >
        ...
        <button 
            x-show="activeMenuCategory && !activeMenuCategory.deleted_at"
            @click="viewDetail(activeMenuCategory.id); closeMenu()"
        ...
        <button 
            x-show="activeMenuCategory && !activeMenuCategory.deleted_at"
            @click="openEditModal(activeMenuCategory)"
        ...
        <div x-show="activeMenuCategory && !activeMenuCategory.deleted_at">
            <div class="h-px bg-surface-100 dark:bg-surface-700/50 my-1 mx-2"></div>
            <button 
                @click="deleteCategory(activeMenuCategory.id); closeMenu()"
        ...
        <div x-show="activeMenuCategory && activeMenuCategory.deleted_at">
            <div class="h-px bg-surface-100 dark:bg-surface-700/50 my-1 mx-2"></div>
            <button 
                @click="restoreCategory(activeMenuCategory.id); closeMenu()"
            ...
            <button 
                @click="forceDeleteCategory(activeMenuCategory.id); closeMenu()"
                class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors group"
            >
                <i data-lucide="trash-2" class="w-4 h-4 group-hover:scale-110 transition-transform"></i>
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\categories\partials\action-menu.blade.php ENDPATH**/ ?>