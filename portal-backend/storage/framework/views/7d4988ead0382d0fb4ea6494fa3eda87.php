...
toggleSelectAll() {
    const currentPageIds = this.tags.map(t => t.id);
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                const response = await fetch('<?php echo e(route("tags.bulk-destroy")); ?>', {
                    method: 'DELETE',
                    headers: {
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                const response = await fetch('<?php echo e(route("tags.bulk-restore")); ?>', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json',
                ...
                const result = await response.json();
                if (result.success) { this.selectedIds = []; this.fetchTags(); showToast('success', result.message); window.dispatchEvent(new CustomEvent('trash-updated')); } 
    ...
    showConfirm(
        ...
        async () => {
            try {
                ...
                const response = await fetch('<?php echo e(route("tags.bulk-force-delete")); ?>', {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json',
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\tags\partials\scripts\bulk-actions.blade.php ENDPATH**/ ?>