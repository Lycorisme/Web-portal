...
<section id="comments" class="mt-8 sm:mt-10 lg:mt-14"
    x-data="commentsSection(<?php echo e($article->id); ?>, <?php echo e(auth()->id() ?? 'null'); ?>, <?php echo e(json_encode($article->visibleComments->map(function($c) {
        return [
            'id' => $c->id,
            'user_id' => $c->user_id,
        ...
        <div class="mb-6 sm:mb-8">
            ...
            <?php else: ?>
                <div class="p-4 sm:p-6 bg-slate-950/50 border border-dashed border-slate-700 rounded-lg sm:rounded-xl text-center">
                    ...
                    <a href="<?php echo e(route('login')); ?>" class="inline-block px-5 sm:px-6 py-2 sm:py-2.5 bg-gradient-to-r from-emerald-600 to-teal-500 text-white text-xs sm:text-sm font-bold rounded-lg sm:rounded-xl shadow-lg shadow-emerald-500/20 hover:from-emerald-500 hover:to-teal-400 transition-all">
                        Login
                    </a>
                </div>
                            ...
                            <div x-data="{ showReply: false, replyText: '', isReplying: false, replyError: '' }">
                                ...
                                <div x-show="showReply" 
                                    ...
                                    <form @submit.prevent="
                                        ...
                                        isReplying = true;
                                        replyError = '';
                                        fetch(`<?php echo e(url('/p/comment')); ?>/${comment.id}/reply`, {
                                            method: 'POST',
                                            headers: {
    ...
    <?php echo $__env->make('public.article.partials.edit-comment-modal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
function commentsSection(articleId, currentUserId, initialComments) {
    return {
        ...
        async submitComment() {
            ...
            try {
                const response = await fetch(`<?php echo e(url('/p/artikel')); ?>/${this.articleId}/comment`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
        ...
        deleteComment(id, parentId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`<?php echo e(url('/p/comment')); ?>/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                            'Accept': 'application/json'
        ...
        async submitEdit() {
            ...
            try {
                const response = await fetch(`<?php echo e(url('/p/comment')); ?>/${this.editCommentId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\public\article\partials\comments-section.blade.php ENDPATH**/ ?>