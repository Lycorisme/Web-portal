        ...
        <div class="space-y-1">
            <template x-for="(cmd, index) in systemCommands" :key="cmd.id">
                <button @click="executeCommand(cmd)"
                        ...
                        class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 group hover:bg-surface-100 dark:hover:bg-surface-800 w-full text-left animate-slide-in-right"
                    ...
                    <div :class="selectedIndex === index ? 'bg-theme-gradient shadow-lg shadow-theme-500/25' : 'bg-surface-200 dark:bg-surface-700'"
                        ...
                        <i :data-lucide="cmd.isDynamic ? (darkMode ? cmd.icon : cmd.iconAlt) : cmd.icon"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div :class="selectedIndex === index ? 'text-theme-600 dark:text-theme-400' : 'text-surface-800 dark:text-white'" 
                             ...
                             x-text="cmd.isDynamic ? (darkMode ? cmd.label : cmd.labelAlt) : cmd.label"></div>
                        
                        <div class="text-sm text-surface-500 dark:text-surface-400" 
                             x-text="cmd.isDynamic ? (darkMode ? cmd.description : cmd.descriptionAlt) : cmd.description"></div>
                    </div>
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\partials\command-palette\commands-mode.blade.php ENDPATH**/ ?>