...
async restoreItem(item) {
    ...
    try {
        ...
        const url = `<?php echo e(url('trash')); ?>/${item.type}/${item.id}/restore`;
...
async forceDeleteItem(item) {
    ...
    try {
        // Manual URL construction to avoid encoding issues with route() placeholders
        const url = `<?php echo e(url('trash')); ?>/${item.type}/${item.id}/force`;
...
async emptyTrash() {
    ...
    try {
        const response = await fetch('<?php echo e(route("trash.empty")); ?>', {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Accept': 'application/json',
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\trash\partials\scripts\crud.blade.php ENDPATH**/ ?>