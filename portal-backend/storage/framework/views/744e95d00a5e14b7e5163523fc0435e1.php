...
<div class="rounded-3xl p-6 md:p-8 bg-slate-900/60 backdrop-blur-md border border-white/5 ring-1 ring-white/5 shadow-xl relative overflow-hidden group">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($activeSessions->count() > 0): ?>
        <div class="space-y-4 relative z-10">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $activeSessions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $session): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex items-center gap-5 p-4 rounded-2xl transition-all duration-300 <?php echo e($session->is_current ? 'bg-emerald-500/10 border border-emerald-500/30 shadow-lg shadow-emerald-500/5' : 'bg-slate-950/40 border border-slate-800/80 hover:bg-slate-900/60'); ?>">
                    <div class="w-14 h-14 rounded-xl <?php echo e($session->is_current ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-900 text-slate-500'); ?> flex items-center justify-center text-2xl border border-white/5 shadow-inner shrink-0">
                        <i class="fas <?php echo e($session->device->icon); ?>"></i>
                    </div>
                    
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-3 mb-1">
                            <h4 class="font-bold text-base <?php echo e($session->is_current ? 'text-white' : 'text-slate-300'); ?>">
                                <?php echo e($session->device->browser); ?> di <?php echo e($session->device->os); ?>
                        ...
                        <div class="flex items-center gap-4 text-xs font-medium text-slate-500">
                            <span class="flex items-center gap-1.5" title="IP Address">
                                <i class="fas fa-network-wired text-slate-600"></i> <?php echo e($session->ip_address); ?>

                            </span>
                            <span class="w-1 h-1 rounded-full bg-slate-700"></span>
                            <span class="flex items-center gap-1.5" title="Terakhir aktif">
                                <i class="far fa-clock text-slate-600"></i> <?php echo e($session->last_activity->diffForHumans()); ?>
    ...
    <div class="mt-8 pt-8 border-t border-white/5 relative z-10">
        <form id="form-logout-all" @submit.prevent="submitForm('form-logout-all', '<?php echo e(route('public.profile.logout-all')); ?>')">
...
<div class="rounded-3xl p-6 md:p-8 bg-slate-900/60 backdrop-blur-md border border-white/5 ring-1 ring-white/5 shadow-xl relative overflow-hidden group mt-8">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($loginHistory->count() > 0): ?>
        <div class="overflow-hidden rounded-2xl border border-slate-800/80 relative z-10">
            <table class="w-full text-left text-sm border-collapse">
                <tbody class="divide-y divide-slate-800/80 bg-slate-950/30">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $loginHistory; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="group/row hover:bg-white/5 transition-colors">
                            <td class="p-4 pl-6">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0
                                        <?php if($log->action === 'LOGIN'): ?> bg-emerald-500/10 text-emerald-400
                                        <?php elseif($log->action === 'LOGOUT'): ?> bg-slate-700/30 text-slate-400
                                        <?php elseif($log->action === 'LOGIN_FAILED'): ?> bg-rose-500/10 text-rose-400
                                        <?php else: ?> bg-amber-500/10 text-amber-400 <?php endif; ?>">
                                        <i class="fas text-xs
                                            <?php if($log->action === 'LOGIN'): ?> fa-sign-in-alt
                                            <?php elseif($log->action === 'LOGOUT'): ?> fa-power-off
                                            <?php elseif($log->action === 'LOGIN_FAILED'): ?> fa-times
                                            <?php else: ?> fa-key <?php endif; ?>"></i>
                                    </div>
                            ...
                            <td class="p-4 pr-6 text-right">
                                <div class="text-slate-400 font-medium text-xs"><?php echo e($log->created_at->format('d M, Y')); ?></div>
                                <div class="text-[10px] text-slate-600 font-bold uppercase tracking-widest"><?php echo e($log->created_at->format('H:i')); ?></div>
                            </td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </tbody>
...
<?php /**PATH C:\laragon\www\web-portal\portal-backend\resources\views\public\profile\partials\security-tab.blade.php ENDPATH**/ ?>