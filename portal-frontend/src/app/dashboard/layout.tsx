...
export default function DashboardLayout({
    ...
    // Toast handler - with animation timing
    const showToast = useCallback((message: string, type: ToastData["type"]) => {
        setToast({ show: true, message, type });
        // Give time for toast to be visible, then trigger exit animation
    ...
    // Control floating bar visibility with exit animation - only show on settings pages
    const isSettingsPage = pathname.startsWith('/dashboard/settings');

    useEffect(() => {
        if (globalHasChanges && isSettingsPage) {
    ...
    }, [globalHasChanges, floatingBarVisible, isSettingsPage]);
    ...
    // Loading state - Use static classes to prevent hydration mismatch
    if (!user) {
        return (
            <div
            ...
            >
                <div className="flex flex-col items-center gap-4">
                    <div
                        ...
                        suppressHydrationWarning
                    ></div>
                    <p className="text-sm text-slate-400" suppressHydrationWarning>Memuat...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className={`font-[family-name:var(--font-inter)] antialiased ${isDarkMode ? 'dark-mode' : ''}`}>
            <div className="flex h-screen overflow-hidden">
                {/* Persistent Sidebar - This never re-renders on navigation */}
                <Sidebar
                    user={user}
                ...
                >
                    ...
                    <div className="flex-1 overflow-y-auto p-6 lg:p-8">
                        {/* Key based on pathname forces re-mount and triggers page-enter animation */}
                        <div key={pathname} className="max-w-6xl mx-auto page-enter">
                            {children}
                        </div>
                    </div>
                </main>
            ...
            {/* Floating Save Bar - Only visible on settings pages with changes */}
            {floatingBarVisible && (
                <div className={`fixed bottom-6 left-0 right-0 z-50 px-4 sm:px-6 flex justify-center ${isFloatingBarExiting ? 'pointer-events-none' : ''}`}>
                    <div
                        className={`w-full max-w-4xl backdrop-blur-xl rounded-2xl border transition-all duration-300 ${isFloatingBarExiting ? 'animate-slide-up-exit' : 'animate-slide-up-enter'} ${isDarkMode
                            ? 'border-slate-700'
                            : 'bg-white border-slate-100'
                            }`}
                        style={{
                            background: isDarkMode
                                ? `linear-gradient(to right, ${theme.sidebar}F2, color-mix(in srgb, ${theme.sidebar}F2, black 20%))`
                                : undefined,
                            boxShadow: isDarkMode
                                ? `0 20px 40px -10px rgba(0,0,0,0.5), 0 0 0 1px ${theme.accent}20`
                                : `0 20px 40px -10px ${theme.accent}20, 0 4px 16px ${theme.accent}10`,
                            borderColor: isDarkMode ? `${theme.softTint}20` : undefined
                        }}
                    >
                        ...
                        <div className="px-4 py-3 sm:px-6 sm:py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="flex items-center gap-4 w-full sm:w-auto">
                                ...
                                <div className="min-w-0 flex-1">
                                    <p className={`text-sm font-bold truncate ${isDarkMode ? 'text-slate-100' : 'text-slate-800'}`}>
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                <button
                                    ...
                                    className={`flex-1 sm:flex-none px-5 py-2.5 rounded-xl text-sm font-bold transition-all hover:scale-105 ${isDarkMode
                                        ? 'bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white border border-slate-700'
                                        : 'bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 border border-slate-200'
                                        }`}
                                    style={{
                                        backgroundColor: isDarkMode ? `color-mix(in srgb, ${theme.sidebar}, white 10%)` : undefined,
